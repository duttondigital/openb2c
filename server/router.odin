...
get_page_html :: proc(path: string) -> (content: string, ok: bool) {
    ...
    // FIXME: route gets scrambled here - memory issue
    route := path == "/" ? "/home" : path
    filename := fmt.tprintf("%s%s%s", BASE_DIR, route, ".html")

    fmt.println(route, path, filename)
    bytes := os.read_entire_file_from_filename(filename) or_return
    // key := route[:1] // TODO: derive key/title from file metadata

    return render_page("home", string(bytes), "home"), true

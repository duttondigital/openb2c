...
Production deployments should use `PAYMENT_PROVIDER=stripe` with `PAYMENT_API_KEY` set to a Stripe secret key and `PAYMENT_WEBHOOK_SECRET` set for signed callbacks. `PAYMENT_PROVIDER=local` and `PAYMENT_PROVIDER=fake` remain available for tests and development; production-like fake-provider runs must set `ALLOW_FAKE_PROVIDERS=true`.

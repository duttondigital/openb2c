...
import { envVarSpecs, requiredProductionEnvVars } from "./config";
...
export function genRoutes(schema: Schema): string {
  ...
  const envSpecs = envVarSpecs(schema).map(spec => ({
    name: spec.name,
    requiredInProduction: spec.requiredInProduction,
    secret: spec.secret,
  }));
  const declaredIntegrations = Object.entries(schema.integrations || {}).map(([name, integration]) => ({
    name,
    provider: integration.provider,
    env: Object.entries(integration.env || {}).map(([envName, env]) => ({
      name: envName,
...
import { APP_CONFIG, DB_PATH, PRODUCTION, REGISTRY_PRIVATE_KEY, REGISTRY_PUBLIC_KEY, REQUIRE_LOCAL_CERTIFICATE_REGISTRY, bootstrapRuntime, initRegistryPublicKey, log, migrationDiagnostics } from "./runtime";
...
const ENV_SPECS = ${JSON.stringify(envSpecs, null, 2)} as const;
const DECLARED_INTEGRATIONS = ${JSON.stringify(declaredIntegrations, null, 2)} as const;
...
function envStatus(spec: { name: string; requiredInProduction: boolean; secret: boolean }) {
  return {
    name: spec.name,
    requiredInProduction: spec.requiredInProduction,
    secret: spec.secret,
    configured: Boolean(process.env[spec.name]),
  };
}

function integrationStatus() {
  return DECLARED_INTEGRATIONS.map(integration => ({
    name: integration.name,
    provider: integration.provider,
    env: integration.env.map(envStatus),
  }));
}
...
function startupDiagnostics() {
  return {
    ...
    config: {
      ...
      wildcardCors: CORS_ORIGINS.includes("*"),
      routeTimeoutMs: ROUTE_TIMEOUT_MS,
      maxRequestBodyBytes: MAX_REQUEST_BODY_BYTES,
      maxPageLimit: MAX_PAGE_LIMIT,
      registryMode: REGISTRY_PRIVATE_KEY ? "local_private_key" : REGISTRY_PUBLIC_KEY ? "external_public_key" : "ephemeral",
      fakeProvidersAllowed: ALLOW_FAKE_PROVIDERS,
    },
    migrations: migrationDiagnostics(),
    integrations: integrationStatus(),
    env: ENV_SPECS.map(envStatus),
  };
}
...
log("info", "startup diagnostics", startupDiagnostics());

...
function route() {
  ...
  if ((m = hash.match(/^\/([a-z_]+s)\/new$/))) {
    ...
    content.innerHTML = `<ob-entity-form entity="${entity}" mode="create"></ob-entity-form>`;
    return;
  }

  if ((m = hash.match(/^\/([a-z_]+s)\/(\d+)\/edit$/))) {
    const entity = m[1].replace(/s$/, "");
    content.innerHTML = `<ob-entity-form entity="${entity}" mode="edit" record-id="${m[2]}"></ob-entity-form>`;
    return;
  }

  if ((m = hash.match(/^\/([a-z_]+s)\/(\d+)$/))) {
    const entity = m[1].replace(/s$/, "");
    content.innerHTML = `<ob-entity-detail entity="${entity}" record-id="${m[2]}"></ob-entity-detail>`;
    return;
  }

  if ((m = hash.match(/^\/([a-z_]+s)$/))) {
    const entity = m[1].replace(/s$/, "");
    content.innerHTML = `<ob-entity-list entity="${entity}"></ob-entity-list>`;
    return;
  }
  ...
  content.innerHTML = `<p style="padding:40px;color:#64748b">Select an entity from the sidebar.</p>`;
}
